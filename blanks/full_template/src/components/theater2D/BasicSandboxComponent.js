...
import {colors} from "../../../engine/utils/colors.js";
...
@layer page{
    ...
    [data-id=canvases].active > canvas:first-child {
        border: 3px solid var(--cr5);
    }
    #BasicSandboxComponent {
       
        .tools {
            position: absolute;
            top: 0;
            background-color: var(--cc15);
            padding: 2px 10px;
        }
    }
} 
...
export class BasicSandboxComponent extends ReaComponent {
    create() {
        ...
        this.camera = new Camera(this.register);

        /**@type {LoopManager} */
        this.loopman = this.register.loopman;

        const regConf = this.register.config;
    ...
    mousemanConfigured(){
        ...
        this.mouseman.addListener('drag', (mouse) => {
            ...
            if (!this.mouseman.draggableOn || !this.isPlaying || !this.isActiveCanvasUI) return;
        ...
        this.mouseman.addListener('wheel', (mouse, event) => {
            ...
            if (!this.mouseman.scalingOn || !this.isPlaying || !this.isActiveCanvasUI) return;
    ...
    get isActiveCanvasUI () {
        return this.layerman.ui.canvas === document.activeElement
    }
    get isPlaying() {
        return this.register.state.status === STATUSES.PLAYING
    }
    ...
    loopmanConfigured(){
        ...
        this.loopman.addLoop( 'sandbox', loop())
