...
export class SelectTransformTarget {
    ...
    setup({renderer, camera, scene, onChange, onDragging, onSelect, enabled = false}) {
        ...
        this.renderer.domElement.addEventListener('click', (event) => {
            if (!this.enabled) return;
            if (Register.instance.controlManager.keyboardManager.keys.shiftKey)
                onClick (event)
        })
    ...
    _setTransformControls() {
        ...
        Register.instance.controlManager.keyboardManager.onKeyJust('Digit1', () => transformControls.setMode('translate'))
        Register.instance.controlManager.keyboardManager.onKeyJust('Digit2', () => transformControls.setMode('rotate'))
        Register.instance.controlManager.keyboardManager.onKeyJust('Digit3', () => transformControls.setMode('scale'))
        Register.instance.controlManager.keyboardManager.onKeyJust('Digit4', () => {
            if (this.intersectedObject) {
                transformControls.space = transformControls.space === 'world' ? 'local' : 'world'
                this.onChange({}, this.intersectedObject)
            }
        })
        Register.instance.controlManager.keyboardManager.onKeyJust('Digit5', () => transformControls.detach())
