...
rcom = new ReaComponent({}, {
    ...
    parent: null,
    template: `div.ReaComponent: "{{title}}" {{game.id}} - {{game.name}}`,
})

// set new state
rcom.setState('game.fuel', String(fuel));
...
div.absolute.top
  div: "{{title}}"
  div[data-id=tools]
    div.button[onclick=@seyHello]: "Sey Hello"
