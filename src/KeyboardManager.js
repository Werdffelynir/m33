...
export class KeyboardManager extends IManager {
    ...
    configured({keymap, onlyfocus, target}) {
        ...
        this._keys = {...this.keymap} 
        Object.keys(this._keys).forEach(name => this._keys[name] = false)
        ...
        this._modifiersJustPressed = {};
        this._modifiersState = new Map();
    ...
    // keymap simpled object {up: bool, left: bool} 
    get keys () {
        return this._keys;
    }
    ...
    _handleKey(event, pressed) {
        ...
        this._modifiersState.set(event.code, pressed);
        const listenerCallbacks = this.listeners.get(event.code) || this.listeners.get(event.key);

        if (pressed) {
            this._modifiers[event.key] = true;
            this._modifiers[event.code] = true;
            this._modifiers.shiftKey = event.shiftKey;
            this._modifiers.ctrlKey = event.ctrlKey;
            this._modifiers.altKey = event.altKey;
            ...
            this._modifiers.shiftKey = false;
            this._modifiers.ctrlKey = false;
            this._modifiers.altKey = false;
    ...
    isPressed(code, callback = null, options = null) {
        let keyIsPressed = this._modifiersState.has(code) && this._modifiersState.get(code) === true;

        if (options)
            for (const [okey, oval] of Object.entries(options)) {
                let _keyIsPressed = this._modifiers?.[okey] && this._modifiers?.[code] === oval;
                if (keyIsPressed)
    ...
    onKeyJust(code, callback) {
        this.onKey(code, (pressed, event) => {
            if(event.type === 'keydown') {
                if (this._modifiersJustPressed?.[code]) return;
                callback(this._modifiers, event)
                this._modifiersJustPressed[code] = true;
            } else {
                this._modifiersJustPressed[code] = false;
            }
        });
    }

    clear() {
        this._modifiersState.clear();
