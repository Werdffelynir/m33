...
export class AssetLoader extends IManager {
    ...
    async loadImage(name, url, cached = true) {
        ...
        if (!this.cache) {
            throw new Error("AssetLoader is not configured");
        }
    ...
    async loadJSON(name, url, cached = true) {
        ...
        try {
            ...
            throw new Error(`loadJSON error: ${url}\n${err.message}`);
        }
    ...
    async loadText(name, url, cached = true) {
        ...
        if (!response.ok) {
            throw new Error(`Text file load failed: ${url} (${response.status})`);
        }
    ...
    async loadBlob(name, url, cached = true) {
        ...
        if (!response.ok) {
            throw new Error(`Blob load failed: ${url} (${response.status})`);
        }
